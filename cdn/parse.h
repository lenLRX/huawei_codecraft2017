...
void parse2(char* graph[MAX_EDGE_NUM], int line_num,Graph& G){
	int i = 0;
	//1st pass
	parse_first_line(G,graph[0]);
	...
	for(;;i++){
		...
		G.add_Edge_without_Reverse(from,to,bandwidth,cost);
		G.add_Edge_without_Reverse(to,from,bandwidth,cost);
	}
